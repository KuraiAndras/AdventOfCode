...
static int GetSegmentValue(SegmentData segmentData)
{
    ...
    var d = bd.Intersect(dg).First();

    var b = bd.Except(new[] { d }).First();

    var g = dg.Except(new[] { d }).First();

    var zero = segmentData.Wirings.Single(w => !w.Contains(d) && w != one && w != seven);

    var e = zero.Except(seven).Except(new[] { b, g }).First();

    var two = segmentData.Wirings.Single(w => w.Length == 5 && w.Contains(e));

    var c = two.Except(new[] { a, d, e, g }).First();

    var f = one.Except(new[] { c }).First();
...
{
    if (segment.SetEquals(new[] { a, b, c, e, f, g })) return 0;
    if (segment.SetEquals(new[] { c, f })) return 1;
    if (segment.SetEquals(new[] { a, c, d, e, g })) return 2;
    if (segment.SetEquals(new[] { a, c, d, f, g })) return 3;
    if (segment.SetEquals(new[] { b, c, d, f })) return 4;
    if (segment.SetEquals(new[] { a, b, d, f, g })) return 5;
    if (segment.SetEquals(new[] { a, b, d, e, f, g })) return 6;
    if (segment.SetEquals(new[] { a, c, f })) return 7;
    if (segment.SetEquals(new[] { a, b, c, d, e, f, g })) return 8;
    if (segment.SetEquals(new[] { a, b, c, d, f, g })) return 9;
...
{
    public static bool SetEquals<T>(this IEnumerable<T> source, IEnumerable<T> other) => new HashSet<T>(source).SetEquals(other);
}
